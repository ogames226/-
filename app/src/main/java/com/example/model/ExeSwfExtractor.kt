...
/**
 * High-performance binary parser for extracting Flash SWF payloads from Windows Flash Projectors (.exe)
...
object ExeSwfExtractor {
    ...
    /**
     * Extracts an SWF stream from raw bytes and optionally writes it to the app's cache directory.
     */
    fun extractSwf(bytes: ByteArray, context: Context? = null, outputFileName: String = "extracted_game.swf"): ExtractionResult {
        ...
        if (isDirectSwf(bytes)) {
            ...
            Log.d(TAG, "Direct SWF recognized: $comp v$version (${bytes.size} bytes, uncompressed target: $declaredSize)")
        ...
        if (matches.isNotEmpty()) {
            // Check matches from last to first (embedded payload in EXEs is usually towards the end)
            for (offset in matches.reversed()) {
                ...
                if (version in 1..40) {
                    ...
                    if (extractedBytes.size >= 8) {
                        Log.d(TAG, "Extracted SWF via stream scan at offset $offset: $comp v$version (${extractedBytes.size} bytes)")
        ...
        return ExtractionResult(
            isSuccess = false,
            swfBytes = null,
            errorMessage = "No valid SWF stream (FWS/CWS/ZWS) or Flash Projector signature found in file"
        )
    }

    /**
     * Reads all bytes from an InputStream and extracts the SWF.
     */
