...
open class RuffleEngineWebView @JvmOverloads constructor(
    ...
    companion object {
        private const val TAG = "RuffleEngineWebView"
    ...
    private fun setupWebView() {
        ...
        webViewClient = object : WebViewClient() {
            override fun shouldInterceptRequest(view: WebView?, request: WebResourceRequest?): WebResourceResponse? {
                ...
                // First check WebViewAssetLoader
                ...
                // Check if requesting the current SWF binary payload
                if (urlStr.contains("game.swf") || urlStr.contains("current_game.swf") ||
                    ...
                    if (bytes != null && bytes.isNotEmpty()) {
                        Log.d(TAG, "Intercepted SWF request: serving ${bytes.size} bytes for $urlStr")
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                Log.d(TAG, "Ruffle page finished loading: $url")
                isHtmlLoaded = true
    ...
    fun loadIndexHtml() {
        isHtmlLoaded = false
        // Load via appassets domain to guarantee CORS WebAssembly and worker support
        loadUrl(INDEX_URL)
    }
